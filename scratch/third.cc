...
void monitor_buffer(FILE *qlen_output, NodeContainer *n) {
  for (uint32_t i = 0; i < n->GetN(); i++) {
    if (n->Get(i)->GetNodeType() == 1) {  // is switch
      ...
      // std::cout << "Switch: " << i << " num devices: " << sw->GetNDevices()
      // << std::endl;
      for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
        // std::cout << " device: " << j << " type: " <<
        // sw->GetDevice(j)->GetNode()->GetId() << std::endl;
        uint32_t size = 0;
...
void CalculateRoute(Ptr<Node> host) {
  ...
  for (int i = 0; i < (int)q.size(); i++) {
    ...
    for (auto it = nbr2if[now].begin(); it != nbr2if[now].end(); it++) {
      ...
      if (dis.find(next) == dis.end()) {
        ...
        // we only enqueue switch, because we do not want packets to go through
        // host as middle point
  ...
  for (auto it : bw) pairBw[it.first][host] = it.second;
}
...
/* Reads parameters in protocol file. */
void read_protocol_file() {
  ...
  while (!conf.eof()) {
    ...
    // 	rate_decrease_interval = v;
    // 	std::cout << "RATE_DECREASE_INTERVAL\t\t" << rate_decrease_interval <<
    // "\n"; }else if (key.compare("MIN_RATE") == 0){ 	conf >> min_rate;
    // 	std::cout << "MIN_RATE\t\t" << min_rate << "\n";
    // }else if (key.compare("FCT_OUTPUT_FILE") == 0){
    // 	conf >> fct_output_file;
    // 	std::cout << "FCT_OUTPUT_FILE\t\t" << fct_output_file << '\n';
    ...
    // 	conf >> link_down_time >> link_down_A >> link_down_B;
    // 	std::cout << "LINK_DOWN\t\t\t\t" << link_down_time << ' '<< link_down_A
    // << ' ' << link_down_B << '\n'; }else if (key.compare("ENABLE_TRACE") ==
    // 0){ 	conf >> enable_trace; 	std::cout << "ENABLE_TRACE\t\t\t\t" <<
    // enable_trace << '\n'; }else if (key.compare("KMAX_MAP") == 0){ 	int n_k
    // ; 	conf >> n_k; 	std::cout << "KMAX_MAP\t\t\t\t"; 	for (int i = 0; i < n_k;
    // i++){ 		uint64_t rate; 		uint32_t k; 		conf >> rate >> k; 		rate2kmax[rate] = k;
    // 		std::cout << ' ' << rate << ' ' << k;
    // 	}
    // 	std::cout<<'\n';
    // }else if (key.compare("KMIN_MAP") == 0){
...
int main(int argc, char *argv[]) {
...
#endif
  {
    ...
    while (!conf.eof()) {
      ...
      /* Start of common_config parameters */
      if (key.compare("COMMON_CONFIG") == 0) {
        uint32_t v;
        conf >> v;
        common_config = v;
        if (common_config)
          std::cout << "COMMON_CONFIG\t"
                    << "Yes"
                    << "\n";
        else
          std::cout << "COMMON_CONFIG\t"
      ...
      // 	if (use_dynamic_pfc_threshold)
      // 		std::cout << "USE_DYNAMIC_PFC_THRESHOLD\t" << "Yes" <<
      // "\n"; 	else 		std::cout << "USE_DYNAMIC_PFC_THRESHOLD\t" <<
      // "No" << "\n";
      // }
      // else if (key.compare("CLAMP_TARGET_RATE") == 0)
      // {
      ...
      // 	conf >> v;
      // 	alpha_resume_interval = v;
      // 	std::cout << "ALPHA_RESUME_INTERVAL\t\t" <<
      ...
      // 	conf >> v;
      // 	rate_decrease_interval = v;
      // 	std::cout << "RATE_DECREASE_INTERVAL\t\t" <<
      // rate_decrease_interval << "\n"; }else if (key.compare("MIN_RATE") ==
      // 0){ 	conf >> min_rate; 	std::cout << "MIN_RATE\t\t" << min_rate
      // << "\n"; }else if (key.compare("FCT_OUTPUT_FILE") == 0){ 	conf >>
      // fct_output_file; 	std::cout << "FCT_OUTPUT_FILE\t\t\t" <<
      // fct_output_file << '\n'; }else if (key.compare("HAS_WIN") == 0){ 	conf
      // >> has_win; 	std::cout << "HAS_WIN\t\t" << has_win << "\n"; }else if
      // (key.compare("GLOBAL_T") == 0){ 	conf >> global_t; 	std::cout <<
      // "GLOBAL_T\t\t" << global_t << '\n'; }else if (key.compare("MI_THRESH")
      // == 0){ 	conf >> mi_thresh; 	std::cout << "MI_THRESH\t\t" << mi_thresh <<
      // '\n'; }else if (key.compare("VAR_WIN") == 0){ 	uint32_t v; 	conf >> v;
      // 	var_win = v;
      // 	std::cout << "VAR_WIN\t\t" << v << '\n';
      // }else if (key.compare("FAST_REACT") == 0){
      // 	uint32_t v;
      ...
      // }else if (key.compare("LINK_DOWN") == 0){
      // 	conf >> link_down_time >> link_down_A >> link_down_B;
      // 	std::cout << "LINK_DOWN\t\t\t\t" << link_down_time << ' '<<
      // link_down_A << ' ' << link_down_B << '\n'; }else if
      // (key.compare("ENABLE_TRACE") == 0){ 	conf >> enable_trace; std::cout
      // << "ENABLE_TRACE\t\t\t\t" << enable_trace << '\n'; }else if
      // (key.compare("KMAX_MAP") == 0){ 	int n_k ; 	conf >> n_k;
      // std::cout << "KMAX_MAP\t\t\t\t"; 	for (int i = 0; i < n_k; i++){
      // uint64_t rate; 		uint32_t k; 		conf >> rate >> k; 		rate2kmax[rate] = k;
      // 		std::cout << ' ' << rate << ' ' << k;
      // 	}
      // 	std::cout<<'\n';
      // }else if (key.compare("KMIN_MAP") == 0){
  ...
  for (uint32_t i = 0; i < node_num; i++) {
    if (n.Get(i)->GetNodeType() == 0) {  // is server
      ...
      serverAddress[i] = Ipv4Address(0x0b000001 + ((i / 256) * 0x00010000) +
                                     ((i % 256) * 0x00000100));
    }
  }
  ...
  for (uint32_t i = 0; i < node_num; i++) {
    if (n.Get(i)->GetNodeType() == 1) {  // is switch
      ...
      for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
        ...
        std::cout << "Calculated delay: " << delay << " headroom: " << headroom
                  << std::endl;
  ...
  if (common_config == 1) {
    for (uint32_t i = 0; i < flow_num; i++) {
      ...
      uint32_t YAPS_id;
      flowf >> YAPS_id >> start_time >> src >> dst >> pg >> dport >>
          maxPacketCount;
      NS_ASSERT(n.Get(src)->GetNodeType() == 0 &&
                n.Get(dst)->GetNodeType() == 0);
      port = portNumder[src]++;  // get a new port number
      RdmaClientHelper clientHelper(
          pg, serverAddress[src], serverAddress[dst], port, dport,
          maxPacketCount,
          has_win ? (global_t == 1 ? maxBdp : pairBdp[n.Get(src)][n.Get(dst)])
                  : 0,
